...
class _MainListPageState extends State<MainListPage> {
	...
	void initState() {
		...
		WidgetsBinding.instance.addPostFrameCallback((_) {
			context.read<RestaurantListProvider>().load();
			_loadFavorites();
		});
	}
	...
	Widget build(BuildContext context) {
		...
		return Scaffold(
			appBar: CustomAppBar(title: 'Restaurants', actions: [
				IconButton(
					tooltip: 'Search',
			...
			body: RefreshIndicator(
				onRefresh: () async {
					await context.read<RestaurantListProvider>().load();
					await _loadFavorites();
				},
						...
						Consumer<RestaurantListProvider>(
							builder: (context, provider, _) {
								...
								if (state is ApiLoading<List<Restaurant>>) {
									...
									_error = null;
									_items = const [];
								} else if (state is ApiError<List<Restaurant>>) {
									_loading = false;
									_error = state.error;
									_items = const [];
								} else if (state is ApiSuccess<List<Restaurant>>) {
									_loading = false;
									_error = null;
									_items = state.data;
								}
										...
										child: Center(
											child: Column(
												...
												children: [
													...
													const SizedBox(height: 12),
													CustomButton.secondary(onPressed: () => context.read<RestaurantListProvider>().load(), label: 'Retry'),
												],
											),
										),
									);
								...
								child: Column(
									...
									children: [
										Hero(
											...
											flightShuttleBuilder: (ctx, anim, dir, from, to) => FadeTransition(opacity: anim, child: to),
											child: Material(
												type: MaterialType.transparency,
												child: Text(restaurant.name, style: Theme.of(context).textTheme.titleLarge, maxLines: 1, overflow: TextOverflow.ellipsis),
											),
										),
